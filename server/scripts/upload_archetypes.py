...
def upload_archetypes():
    """16개 유형 데이터를 Firestore에 업로드"""
    db = firestore.client()
    archetypes = get_full_archetypes()

    for arc in archetypes:
        db.collection("archetypes").document(arc["id"]).set(arc)
        print(f"  ✅ {arc['id']} - {arc['alias']['ko']} 업로드 완료")

    print(f"\n🎉 총 {len(archetypes)}개 유형의 완벽한 데이터 프레임워크 업로드 완료!")
