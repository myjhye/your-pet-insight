...
export default function AnalyzingLoader({ lang = 'en' }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gradient-to-b from-[#FFFDF9] via-[#F9FBF9] to-[#FFF7ED] px-6">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-80 h-80 bg-orange-200/20 rounded-full blur-3xl pointer-events-none"></div>

      <div className="relative z-10 w-full max-w-sm flex flex-col items-center text-center">
        {/* 강아지 이미지 애니메이션 컨테이너 */}
        <div className="relative w-40 h-40 mb-6 flex items-center justify-center">
          <div className="absolute inset-0 border-4 border-dashed border-orange-300/40 rounded-full animate-spin" style={{ animationDuration: '10s' }}></div>
          
          <img
            key={imgIndex}
            src={DOG_IMAGES[imgIndex]}
            alt="Analyzing pet"
            className="w-32 h-32 object-contain drop-shadow-xl transition-all duration-200 transform scale-105"
          />
        </div>
